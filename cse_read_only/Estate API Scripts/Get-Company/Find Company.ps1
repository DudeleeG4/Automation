...
Import-Module UKCloud.Logging.psm1 -errorAction silentlyContinue
...
$CompanyNumber = Invoke-GUIForm -Questions "Enter Company Number:"

$Query = @{query = "query findCompanyByDomainId(
  `$securityDomain: [SecurityDomain!],
  `$domainId: [Int!]
) {
  companies(domainIdentifier: `$domainId, securityDomain: `$securityDomain) {
    id
    name
    accounts {
      id
...
}";variables = '{"domainId":' + $CompanyNumber.Answer + ', "SecurityDomain":"' + $ImpactLevel + '"}'}



$R = Invoke-RestMethod -Uri $URL -Headers $dict -Method Post -Body $query -ContentType "application/x-www-form-urlencoded"
$Report = $R.data.companies |% {
	$Company = $_
	Foreach ($Account in $_.Accounts){
		[PSCustomObject]@{
		Company = $Company.Name
		Account = $Account.Name
		Services = $Account.Services.Name -join ";    "		
		}	
	}
}
