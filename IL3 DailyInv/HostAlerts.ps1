...
$esx_all = Get-View -ViewType HostSystem 
$Report=@()
...
"@
 ...
 $OutputFile="C:\inetpub\wwwroot\AlertsOnHosts.html" 
 $OutputFileCSV = "C:\inetpub\wwwroot\AlertsOnHosts.csv"
 $OS = $report | Sort VC,Cluster,Name,Time,Status  | ConvertTo-Html -Title "Test Title" -Head $HeadTest -Body "<div id='title'><center><h1>ESXi Hosts Alerts</h1></div><a href=AlertsOnHosts.csv title=DownloadCSV>Download Data</a><div id='subtitle'><center><h1>Report generated: $DateSub</h1></div><br></center><center><h2>Alerts per Hosts</h2></center>"   #</div><div id='subtitle'>Report generated: $DateSub</div>"
...
If ($env:USERDOMAIN -like "*il2*")
{
Send-MailMessage -To "noc@skyscapecloud.com" -From "ESXiAlertsIL2@skyscapecloud.com" -Subject "ESXi Hosts Alerts $gDate" -SmtpServer rly00001i2 -Body $HBody -BodyAsHtml  -Attachments $OutputFile,$OutputFileCSV #$outfile,$outfile1 
}
Else {
Send-MailMessage -To "noc@skyscapecloud.com" -From "ESXiAlertsIL3@skyscapecloud.com" -Subject "ESXi Hosts Alerts $gDate" -SmtpServer 10.72.81.30 -Body $HBody -BodyAsHtml  -Attachments $OutputFile,$OutputFileCSV #-Attachments $outfile,$outfile1
}
