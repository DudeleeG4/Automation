...
$esx_all = Get-View -ViewType Datastore 
$Report=@()
foreach ($esx in $esx_all){
    foreach($triggered in $esx.TriggeredAlarmState){
        If ($triggered.OverallStatus -like "*" ){
            $lineitem={} | Select Name, AlarmInfo, Status, Time, VC, Cluster, Acknowledged, AcknowledgedByUser, AcknowledgedTime
			$lineitem.VC = $VC = $esx.Client.ServiceURL.trim("https://").Split(".")[0].Trim("/sdk")
			$VCF = "$VC" + "." + $env:USERDNSDOMAIN
            $alarmDef = Get-View -Id $triggered.Alarm -Server $VCF #"vcw00007i3.il3management.local"
			...
			$lineitem.Cluster = $Cluster = (Get-View -Id $esx.Parent -Server $VCF).Name # | Where-Object {$_.Client.ServiceURL -like "*$VC*"} | Select-Object -ExpandProperty Name
            $Report+=$lineitem
        } 
    }
}
$Report
#Get-View -Id $triggered.key
...
"@
 ...
 $OutputFile="C:\inetpub\wwwroot\AlertsOnDatastores.html" 
 $OutputFileCSV = "C:\inetpub\wwwroot\AlertsOnDatastores.csv"
 $OS = $report | Sort VC,Cluster,Name,Time,Status | ConvertTo-Html -Title "Test Title" -Head $HeadTest -Body "<div id='title'><center><h1>Datastores Alerts</h1></div><a href=AlertsOnDatastores.csv title=DownloadCSV>Download Data</a><div id='subtitle'><center><h1>Report generated: $DateSub</h1></div><br></center><center><h2>Alerts per Datastores</h2></center>"   #</div><div id='subtitle'>Report generated: $DateSub</div>"
...
If ($env:USERDOMAIN -like "*il2*")
{
Send-MailMessage -To "noc@skyscapecloud.com" -From "DatastoresAlertsIL2@skyscapecloud.com" -Subject "Datastores Alerts $gDate" -SmtpServer rly00001i2 -Body $HBody -BodyAsHtml  -Attachments $OutputFile,$OutputFileCSV #$outfile,$outfile1 
}
Else {
Send-MailMessage -To "noc@skyscapecloud.com" -From "DatastoresAlertsIL3@skyscapecloud.com" -Subject "Datastores Alerts $gDate" -SmtpServer 10.72.81.30 -Body $HBody -BodyAsHtml  -Attachments $OutputFile,$OutputFileCSV #-Attachments $outfile,$outfile1
}
